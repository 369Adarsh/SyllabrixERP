...
const getBranchId = (req) => (req.user?.role === 'MANAGER' && req.user.branchId) ? req.user.branchId : (req.query.branchId || null);

const getToday   = async (req, res, next) => { try { ok(res, await svc.today(req.tenantId, { branchId: getBranchId(req) })); } catch (e) { next(e); } };
...
const getReport  = async (req, res, next) => { try { ok(res, await svc.report(req.tenantId, req.query.from, req.query.to, req.query.staffId, getBranchId(req))); } catch (e) { next(e); } };
const getSummary = async (req, res, next) => { try { ok(res, await svc.summary(req.tenantId, req.query.from, req.query.to, getBranchId(req))); } catch (e) { next(e); } };
