const svc = require('./creditnotes.service');
const { ok, created } = require('../../utils/response');

const list   = async (req, res, next) => { try { ok(res, await svc.list(req.tenantId, req.query)); } catch (e) { next(e); } };
const get    = async (req, res, next) => { try { ok(res, await svc.get(req.tenantId, req.params.id)); } catch (e) { next(e); } };
const create = async (req, res, next) => { try { created(res, await svc.create(req.tenantId, req.body), 'Credit note created'); } catch (e) { next(e); } };
const status = async (req, res, next) => { try { ok(res, await svc.updateStatus(req.tenantId, req.params.id, req.body.status)); } catch (e) { next(e); } };

module.exports = { list, get, create, status };
